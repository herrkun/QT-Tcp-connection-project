    ...
    ui(new Ui::MyTcpServer)
{
    ...
    ui->edtIP->setText(QNetworkInterface().allAddresses().at(0).toString());   //获取本地IP
...
void MyTcpServer::ReadData()
{
    ...
    for(int i=0; i<tcpClient.length(); i++)
    {
        ...
        // 将发送过来的数据进行分割，后续可以根据这些发送接收的消息做成两个软件之间的相互控制
        QList<QByteArray> buffer_list = buffer.split(' ');
        ui->edtRecv->append(buffer_list[0]);
